    ...
    public class MessageDispatcherComponent : Component
	{
		...
		public void Load()
		{
			...
			foreach (Type type in types)
			{
				...
				if (iMHandler == null)
				{
					ETLog.Error($"message handle {type.Name} 需要继承 IMHandler");
					continue;
				}
				...
				if (opcode == 0)
				{
					ETLog.Error($"消息opcode为0: {messageType.Name}");
					continue;
				}
		...
		public void Handle(Session session, MessageInfo messageInfo)
		{
			...
			{
				ETLog.Error($"消息没有处理: {messageInfo.Opcode} {JsonHelper.ToJson(messageInfo.Message)}");
				return;
			}
