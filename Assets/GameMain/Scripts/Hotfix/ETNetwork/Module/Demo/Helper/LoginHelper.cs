    ...
    public static class LoginHelper
    {
        public static async ETVoid OnLoginAsync(string account)
        {
            try
            {
                ...
                // 创建一个Trinity.Hotfix层的Session, Trinity.Hotfix的Session会通过ETModel层的Session发送消息
                ...
                // 创建一个Trinity.Hotfix层的Session, 并且保存到Trinity.Hotfix.SessionComponent中
                Game.Scene.AddComponent<SessionComponent>().Session = ComponentFactory.Create<Session, ETModel.Session>(gateSession);
				
                G2C_LoginGate g2CLoginGate = (G2C_LoginGate)await SessionComponent.Instance.Session.Call(new C2G_LoginGate() { Key = r2CLogin.Key });

                ETModel.ETLog.Info("登陆gate成功!");
