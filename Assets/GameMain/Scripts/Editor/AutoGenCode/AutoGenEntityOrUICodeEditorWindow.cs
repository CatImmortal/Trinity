...
namespace Trinity.Editor
{
    ...
    public class AutoGenEntityOrUICodeEditorWindow : EditorWindow
    {
        ...
        {
            AutoGenEntityOrUICodeEditorWindow window = GetWindow<AutoGenEntityOrUICodeEditorWindow>(true, "自动生成实体或界面的代码");
        ...
        private void OnGUI()
        {
            ...
            switch (m_TargetType)
            {
                case AutoGeneType.Entity:
                    EditorGUILayout.LabelField(EntityCodePath);
                    break;
                case AutoGeneType.HotfixEntity:
                    EditorGUILayout.LabelField(HotfixEntityCodePath);
                    break;
                case AutoGeneType.UIForm:
                    EditorGUILayout.LabelField(UIFormCodePath);
                    break;
                case AutoGeneType.HotfixUIForm:
                    EditorGUILayout.LabelField(HotfixUIFormCodePath);
                    break;
                default:
                    break;
            }
            ...
            if (GUILayout.Button("生成模板代码", GUILayout.Width(100f)))
            {
                ...
                {
                    case AutoGeneType.Entity:
                        AutoGenEntityCode(false);
                        break;
                    case AutoGeneType.HotfixEntity:
                        AutoGenEntityCode(true);
                        break;
                    case AutoGeneType.UIForm:
                        AutoGenUIFormCode(false);
                        break;
                    case AutoGeneType.HotfixUIForm:
                        AutoGenUIFormCode(true);
                        break;
                    default:
                        break;
                }
            ...
            if (GUILayout.Button("生成简便显示实体方法的代码", GUILayout.Width(200f)))
            {
                ...
                {
                    AutoGenShowEntityCode(false);
                }
                else
                {
                    AutoGenShowEntityCode(true);
                }
        ...
        /// 自动生成实体代码
        /// </summary>
        private void AutoGenEntityCode(bool isHotfix)
        {
            //根据是否为热更新实体来决定一些参数
        ...
        /// 自动生成界面代码
        /// </summary>
        private void AutoGenUIFormCode(bool isHotfix)
        {
            //根据是否为热更新界面来决定一些参数
            string codepath = isHotfix ? HotfixUIFormCodePath : UIFormCodePath;
            string nameSpace = isHotfix ? "Trinity.Hotfix" : "Trinity";
